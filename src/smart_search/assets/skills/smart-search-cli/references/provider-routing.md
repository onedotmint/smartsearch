...
- The standard minimum profile requires one configured provider in each of `main_search`, `docs_search`, and fetch capability for profile diagnostics and `doctor`. Command execution is capability-scoped: concise/synthesized `search` requires `main_search`, explicit lite/off evidence search may use `web_search` or `docs_search` source-only, `fetch` requires `web_fetch`, `map` requires `site_map`, and `research` requires `web_fetch` while discovery capabilities remain optional. Command failures expose `required_capabilities`, `missing_capabilities`, `required_providers`, and `missing_providers`; provider-specific commands require their named provider and same-capability route.
