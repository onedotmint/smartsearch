...
- v0.3.0 retrieval gateway: when Brave or Exa is configured (Tavily for research intent), `search`/`source_discovery` runs Brave/Exa/Tavily through the multi-source gateway: one normalized DiscoveryCandidate model, deterministic URL canonicalization and cross-provider dedup with provenance, reciprocal-rank fusion (RRF, `k=60`), and optional best-effort Jina reranking (`JINA_API_KEY`) that never replaces RRF as the fallback ranking. Provider-native scores are diagnostics only. Setups without Brave/Exa/Tavily keep the exact pre-v0.3.0 source-discovery path.
