...
- Always read back npm before and after publishing with `npm view @onedotmint/smart-search versions --json` and `npm view @onedotmint/smart-search dist-tags --json`. A test release must leave `latest` on the stable version and move only `next` or the explicitly supplied non-`latest` tag.
